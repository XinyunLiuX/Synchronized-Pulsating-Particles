...
batch_omega = 4.8;%[2.1:0.02:2.6]*2;
batch_epsilon = 0.07;
...
% Geometry Parameters
N1 = 20;             % number of particles in a row
N = N1^2;            % total number of particles   
rho = 2;             % density of particles rho = N/L^2, number of particles per area
L = 2^(1/6)*N1;      % domain width
...
perb = 1E-2*ones(N1,1); perb(1:2:end) = -perb(1:2:end);
x0 =  (0:N1-1).'*2^(1/6) + 2^(1/6)/2 + perb; 
y0 =  (0:N1-1).'*2^(1/6) + 2^(1/6)/2; 
