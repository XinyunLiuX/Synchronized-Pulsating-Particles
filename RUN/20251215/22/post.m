...
exportgraphics(gcf, [folder exportfineName, '_mean.png'])

figure
scatter(Omega, Std, 20, 'k', 'filled');
savefig([folder exportfineName, '_std.fig'])
exportgraphics(gcf, [folder exportfineName, '_std.png'])
...
for i = 1:numFile
    ...
    y = output.y(1:10:end, :);
    T = output.T(1:10:end, :);
    phi4 = zeros(size(x,1), 1);
    phi6 = zeros(size(x,1), 1);
    for t = 1:size(T,1)
        [phi4(t), phi6(t)] = bond_order(x(t,:), y(t,:));
    end

    figure
    plot(T, phi4, T, phi6, 'LineWidth', 1.5);
    legend('phi4', 'phi6')
    savefig([folder fileName(1:end-4), '_bond_order.fig'])
    exportgraphics(gcf, [folder fileName(1:end-4), '_bond_order.png'])
end
